...
def upload_document(files, description, category, status, access):
    """Uploads a document to the backend."""
    logger.info(f"Preparing to upload document '{files.name}'.")
    form_data = {
    ...
    try:
        with st.spinner(f"Uploading {files.name}..."):
            response = requests.post(
                UPLOAD_ENDPOINT,
                files={'file': (files.name, files.getvalue(), files.type)},
                data=form_data,
                headers=get_headers()
            )
            response.raise_for_status()
            logger.info(f"Successfully uploaded document '{files.name}'.")
